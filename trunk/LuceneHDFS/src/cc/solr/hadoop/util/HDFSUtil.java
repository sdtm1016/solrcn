...
public class HDFSUtil {
	...
	// private static Log log = new LogFactory(HDFSUtil.class);
			...
			for (FileStatus f : fList) {
				if (null != f) {
					...
					if (f.isDir()) {
						...
						System.out.println(convertSize(f.getLen()) + "\t\t"
								+ relativePath);
					}
				}
			}
	...
	public synchronized static long getLength(FileSystem fs, String path) {
		Path workDir = fs.getWorkingDirectory();
		Path dst = new Path(workDir + "/" + path);
		try {
			return fs.getFileStatus(dst).getLen();
		} catch (IOException e) {
			return 0;
		}
